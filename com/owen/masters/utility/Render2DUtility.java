...
public class Render2DUtility {
	...
	public Render2DUtility() {
		RENDER_ITEM = new RenderItem(Minecraft.getMinecraft().renderEngine,
	...
	public static void drawRect(final Rectangle rectangle, final int color) {
		drawRect(rectangle.x, rectangle.y, rectangle.x + rectangle.width,
				rectangle.y + rectangle.height, color);
	}

	public static void drawRect(final float x, final float y, final float x1,
	...
	public static void drawBorderedRect(final float x, final float y,
			final float x1, final float y1, final float width,
	...
	public static void drawBorderedRect(float x, float y, float x1, float y1,
			final int insideC, final int borderC) {
		enableGL2D();
		x *= 2.0f;
		x1 *= 2.0f;
		y *= 2.0f;
	...
	public static void drawBorderedRectReliant(final float x, final float y,
			final float x1, final float y1, final float lineWidth,
			final int inside, final int border) {
		enableGL2D();
	...
	public static void drawGradientBorderedRectReliant(final float x,
	...
	public static void drawBorderedRect(final Rectangle rectangle,
	...
	public static void drawGradientRect(final float x, final float y,
			final float x1, final float y1, final int topColor,
	...
	public static void drawGradientHRect(final float x, final float y,
			final float x1, final float y1, final int topColor,
			final int bottomColor) {
		enableGL2D();
		GL11.glShadeModel(7425);
	...
	public static void drawGradientRect(final double x, final double y,
			final double x2, final double y2, final int col1, final int col2) {
		GL11.glEnable(3042);
		GL11.glDisable(3553);
		GL11.glBlendFunc(770, 771);
		GL11.glEnable(2848);
	...
	public static void drawGradientBorderedRect(final double x, final double y,
			final double x2, final double y2, final float l1, final int col1,
	...
	public static void glColor(final float alpha, final int redRGB,
			final int greenRGB, final int blueRGB) {
	...
	public static void drawStrip(final int x, final int y, final float width,
			final double angle, final float points, final float radius,
			final int color) {
	...
	public static void drawHLine(float x, float y, final float x1,
	...
	public static void drawRect(final float x, final float y, final float x1,
			final float y1, final float r, final float g, final float b,
			final float a) {
		enableGL2D();
	...
	public static void drawCircle(float cx, float cy, float r,
			final int num_segments, final int c) {
		r *= 2.0f;
		cx *= 2.0f;
		cy *= 2.0f;
	...
	public static void drawSmallString(final String s, final int x,
			final int y, final int color) {
		GL11.glPushMatrix();
		GL11.glScalef(0.5f, 0.5f, 0.5f);
		Minecraft.getMinecraft().fontRendererObj.func_175063_a(s, x * 2, y * 2,
				color);
		GL11.glPopMatrix();
	}

	public static void drawLargeString(final String text, int x, final int y,
			final int color) {
		x *= 2;
		GL11.glPushMatrix();
		GL11.glScalef(1.5f, 1.5f, 1.5f);
		Minecraft.getMinecraft().fontRendererObj.func_175063_a(text, x, y,
				color);
		GL11.glPopMatrix();
	}

	public static ScaledResolution getScaledResolution() {
		return Render2DUtility.scaledResolution;
	}
