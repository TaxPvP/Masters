...
public class Render3DUtility {
	...
	public static Color getRainbow(final long offset, final float fade) {
		...
		final long color = Long.parseLong(Integer.toHexString(Integer
				.valueOf(Color.HSBtoRGB(hue, 1.0f, 1.0f))), 16);
		final Color c = new Color((int) color);
		return new Color(c.getRed() / 255.0f * fade, c.getGreen() / 255.0f
				* fade, c.getBlue() / 255.0f * fade, c.getAlpha() / 255.0f);
	}
	...
	public static void revertGLCap(final int cap) {
		final Boolean origCap = glCapMap.get(cap);
		if (origCap != null) {
			if (origCap) {
				GL11.glEnable(cap);
			} else {
	...
	public static void revertAllCaps() {
		for (final int cap : glCapMap.keySet()) {
			revertGLCap(cap);
		}
	}
	...
	public static void drawOutlinedBoundingBox(
			final AxisAlignedBB par1AxisAlignedBB) {
		final Tessellator var16 = Tessellator.getInstance();
		final WorldRenderer var17 = var16.getWorldRenderer();
		var17.startDrawing(3);
		var17.addVertex(par1AxisAlignedBB.minX, par1AxisAlignedBB.minY,
				par1AxisAlignedBB.minZ);
		var17.addVertex(par1AxisAlignedBB.maxX, par1AxisAlignedBB.minY,
				par1AxisAlignedBB.minZ);
		var17.addVertex(par1AxisAlignedBB.maxX, par1AxisAlignedBB.minY,
		...
		var16.draw();
		var17.startDrawing(3);
		var17.addVertex(par1AxisAlignedBB.minX, par1AxisAlignedBB.maxY,
				par1AxisAlignedBB.minZ);
		var17.addVertex(par1AxisAlignedBB.maxX, par1AxisAlignedBB.maxY,
				par1AxisAlignedBB.minZ);
		var17.addVertex(par1AxisAlignedBB.maxX, par1AxisAlignedBB.maxY,
				par1AxisAlignedBB.maxZ);
		var17.addVertex(par1AxisAlignedBB.minX, par1AxisAlignedBB.maxY,
	...
	public static void renderItemStack(final ItemStack stack, final int x,
			final int y) {
		GL11.glPushMatrix();
		GL11.glDepthMask(true);
		...
		Minecraft.getMinecraft().getRenderItem().func_180450_b(stack, x, y);
		Minecraft
				.getMinecraft()
				.getRenderItem()
				.func_175030_a(Minecraft.getMinecraft().fontRendererObj, stack,
						x, y);
	...
	public static void drawitemStackEnchants(final ItemStack stak, final int x,
			final int y) {
		...
		if (enchants != null) {
			...
			for (int index = 0; index < enchants.tagCount(); ++index) {
				final short id = enchants.getCompoundTagAt(index)
						.getShort("id");
				final short level = enchants.getCompoundTagAt(index).getShort(
						"lvl");
				if (Enchantment.field_180311_a[id] != null) {
					final Enchantment enc = Enchantment.field_180311_a[id];
					final String encName = enc.getTranslatedName(level)
							.substring(0, 2).toLowerCase();
					final String[] ShownEnchants = { "Efficiency",
							"Unbreaking", "Sharpness", "FireAspect", "" };
					Minecraft.getMinecraft().fontRendererObj.func_175063_a(
							String.valueOf(encName) + "§b" + level, x,
							y + ency, -5592406);
					ency += Minecraft.getMinecraft().fontRendererObj.FONT_HEIGHT;
					if (index > 4) {
						Minecraft.getMinecraft().fontRendererObj.func_175063_a(
								"§f+ others", x, y + ency, -5592406);
						break;
					}
				}
			}
	...
	public static final class Stencil {
		...
		public void startLayer() {
			...
			if (this.layers > this.getMaximumLayers()) {
				System.out
						.println("StencilUtil: Reached maximum amount of layers!");
				this.layers = 1;
			}
		}
		...
		public void stopLayer() {
			...
			if (this.layers == 1) {
				...
				final StencilFunc lastStencilFunc = this.stencilFuncs
						.remove(this.layers);
		...
		public void setBuffer() {
			this.setStencilFunc(new StencilFunc(this, this.renderMask ? 519
					: 512, this.layers, this.getMaximumLayers(), 7681, 7680,
					7680));
		}

		public void setBuffer(final boolean set) {
			this.setStencilFunc(new StencilFunc(this, this.renderMask ? 519
					: 512, set ? this.layers : (this.layers - 1), this
					.getMaximumLayers(), 7681, 7681, 7681));
		}

		public void cropOutside() {
			this.setStencilFunc(new StencilFunc(this, 517, this.layers, this
					.getMaximumLayers(), 7680, 7680, 7680));
		}

		public void cropInside() {
			this.setStencilFunc(new StencilFunc(this, 514, this.layers, this
					.getMaximumLayers(), 7680, 7680, 7680));
		}

		public void setStencilFunc(final StencilFunc stencilFunc) {
			GL11.glStencilFunc(StencilFunc.func_func, StencilFunc.func_ref,
					StencilFunc.func_mask);
			GL11.glStencilOp(StencilFunc.op_fail, StencilFunc.op_zfail,
					StencilFunc.op_zpass);
		...
		public void createCirlce(final double x, final double y,
				final double radius) {
			GL11.glBegin(6);
			for (int i = 0; i <= 360; ++i) {
				final double sin = Math.sin(i * 3.141592653589793 / 180.0)
						* radius;
				final double cos = Math.cos(i * 3.141592653589793 / 180.0)
						* radius;
		...
		public void createRect(final double x, final double y, final double x2,
				final double y2) {
			GL11.glBegin(7);
		...
		public static class StencilFunc {
			...
			public StencilFunc(final Stencil paramStencil, final int func_func,
					final int func_ref, final int func_mask, final int op_fail,
					final int op_zfail, final int op_zpass) {
	...
	public static class Camera {
		...
		public Camera(final Entity entity) {
			...
			if (entity instanceof EntityPlayer
				...
				this.setRotationYaw(this.getRotationYaw()
						+ living1.prevCameraYaw
						+ (living1.cameraYaw - living1.prevCameraYaw)
						* this.timer.renderPartialTicks);
				this.setRotationPitch(this.getRotationPitch()
						+ living1.prevCameraPitch
						+ (living1.cameraPitch - living1.prevCameraPitch)
						* this.timer.renderPartialTicks);
		...
		public Camera(final Entity entity, final double offsetX,
			...
			if (entity instanceof EntityPlayer
				...
				this.setRotationYaw(this.getRotationYaw()
						+ player.prevCameraYaw
						+ (player.cameraYaw - player.prevCameraYaw)
						* this.timer.renderPartialTicks);
				this.setRotationPitch(this.getRotationPitch()
						+ player.prevCameraPitch
		...
		public Camera(final double posX, final double posY, final double posZ,
		...
		public static float[] getRotation(final double posX1,
				final double posY1, final double posZ1, final double posX2,
				final double posY2, final double posZ2) {
